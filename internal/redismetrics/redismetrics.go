...
func (r *RedisMetrics) password(ctx context.Context, gceService gceInterface) (secret.String, error) {
	pw := ""
	if r.Config.GetRedisConfiguration().GetPassword() != "" {
		return secret.String(r.Config.GetRedisConfiguration().GetPassword()), nil
	}
	secretCfg := r.Config.GetRedisConfiguration().GetSecret()
...
func (r *RedisMetrics) InitDB(ctx context.Context, gceService gceInterface) error {
	...
	port := r.Config.GetRedisConfiguration().GetPort()
	if port == "" {
		port = defaultPort
	}
...
func (r *RedisMetrics) replicationModeActive(ctx context.Context) bool {
	...
	for _, line := range lines {
		if strings.Contains(line, role) {
			if strings.Contains(line, redisMain) {
				log.CtxLogger(ctx).Debugf("Redis is running as main role.")
				currentRole = main
			} else if strings.Contains(line, redisWorker) {
...
func (r *RedisMetrics) persistenceEnabled(ctx context.Context) bool {
	...
	log.CtxLogger(ctx).Infof("RDB persistence: %v", persistence)
	...
	if saveInfo, ok := persistence[save]; ok && saveInfo != "" {
		log.CtxLogger(ctx).Infof("RDB persistence: %v", saveInfo)
		return true
	}

	// Check AOF persistence
	persistence = r.db.ConfigGet(ctx, appendonly).Val()
	log.CtxLogger(ctx).Infof("AOF persistence: %v", persistence)
