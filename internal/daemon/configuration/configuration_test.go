...
func TestApplyDefaultMySQLConfiguration(t *testing.T) {
	tests := []struct {
		name             string
		cfg              *cpb.Configuration
		isMySQLRunning   bool
		want             *cpb.Configuration
		wantMySQLEnabled bool
	}{
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := ApplyDefaultMySQLConfiguration(tc.cfg, tc.isMySQLRunning)
			if diff := cmp.Diff(tc.want, got, protocmp.Transform()); diff != "" {
				t.Errorf("ApplyDefaultMySQLConfiguration() returned an unexpected diff (-want +got): %v", diff)
			}
		})
	}
}

func TestApplyDefaultOracleConfiguration(t *testing.T) {
	tests := []struct {
		name              string
		cfg               *cpb.Configuration
		isOracleRunning   bool
		want              *cpb.Configuration
		wantOracleEnabled bool
	}{
		{
			name: "Oracle is running",
			cfg: &cpb.Configuration{
				OracleConfiguration: &cpb.OracleConfiguration{
					Enabled: proto.Bool(false),
				},
			...
			want: &cpb.Configuration{
				OracleConfiguration: &cpb.OracleConfiguration{
					// Doesn't actually change the configuration for now because the method is not yet implemented.
					Enabled: proto.Bool(false),
				},
			},
		},
		{
			name: "Oracle is not running",
			cfg: &cpb.Configuration{
				OracleConfiguration: &cpb.OracleConfiguration{
					Enabled: proto.Bool(false),
				},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := ApplyDefaultOracleConfiguration(tc.cfg, tc.isOracleRunning)
			if diff := cmp.Diff(tc.want, got, protocmp.Transform()); diff != "" {
				t.Errorf("ApplyDefaultOracleConfiguration() returned an unexpected diff (-want +got): %v", diff)
			}
		})
	}
}
