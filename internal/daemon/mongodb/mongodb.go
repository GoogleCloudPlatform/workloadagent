...
func (s *Service) Start(ctx context.Context, a any) {
	...
	go (func() {
		for {
			...
			if err != nil {
				log.CtxLogger(ctx).Infow("MongoDB stopping checks for service communication", "error", err)
				return
			}
		}
	})()
...
func (s *Service) checkServiceCommunication(ctx context.Context) error {
	...
	case msg, ok := <-s.CommonCh:
		...
		if msg == nil {
			log.CtxLogger(ctx).Debug("MongoDB workload agent service received nil message on the common channel")
			return nil
		}
