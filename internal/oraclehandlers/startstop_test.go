...
func TestStopDatabase(t *testing.T) {
	tests := []struct {
		...
		sqlResult     *commandlineexecutor.Result
		...
		},
		{
			...
			sqlResult:     &commandlineexecutor.Result{StdOut: shutdownSuccess},
		...
		},
		{
			...
			sqlResult:     &commandlineexecutor.Result{ExitCode: 1, Error: fmt.Errorf("shutdown failed")},
		...
		},
		{
			...
			sqlResult:     &commandlineexecutor.Result{StdOut: alreadyDown},
			wantErrorCode: codepb.Code_OK,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			runSQL = func(ctx context.Context, params map[string]string, query string) *commandlineexecutor.Result {
				return tc.sqlResult
			}
...
func TestStopDatabaseLocked(t *testing.T) {
	...
	runSQL = func(ctx context.Context, params map[string]string, query string) *commandlineexecutor.Result {
		runSQLBlocked <- true
		<-unblockRunSQL
		return &commandlineexecutor.Result{StdOut: shutdownSuccess}
	}
...
func TestStartDatabase(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			defer func() { runSQL = origRunSQL }()
			runSQL = func(ctx context.Context, params map[string]string, query string) *commandlineexecutor.Result {
				if result, ok := tc.sqlQueries[query]; ok {
					return result
				}
				return &commandlineexecutor.Result{ExitCode: 1, Error: fmt.Errorf("unexpected query: %s", query)}
			}
...
func TestStartDatabaseLocked(t *testing.T) {
	...
	runSQL = func(ctx context.Context, params map[string]string, query string) *commandlineexecutor.Result {
		...
		return &commandlineexecutor.Result{StdOut: startupSuccess}
	}
