...
func (h *OracleHandler) lockDatabase(ctx context.Context, command *gpb.Command) (func(), *gpb.CommandResult) {
	...
	if op, ok := h.activeOps[params["oracle_sid"]]; ok {
		...
		if err != nil {
			logger.Warnf("Failed to pack payload: %v", err)
		}
...
func commandResult(ctx context.Context, command *gpb.Command, result *commandlineexecutor.Result, code codepb.Code, message string) *gpb.CommandResult {
	params := command.GetAgentCommand().GetParameters()
	logger := log.CtxLogger(ctx).With("oracle_sid", params["oracle_sid"], "oracle_home", params["oracle_home"], "oracle_user", params["oracle_user"])
	anyPayload, err := anypb.New(&spb.Status{
		Code:    int32(code),
	...
	if result != nil {
		res.ExitCode = int32(result.ExitCode)
