...
const (
	...
	sourceRole            = "source"
	replicaRole           = "replica"
	replicationZonesQuery = "SELECT * FROM information_schema.PROCESSLIST AS p WHERE p.COMMAND = 'Binlog Dump'"
)
...
func (m *MySQLMetrics) getCurrentRole(ctx context.Context) string {
	role := sourceRole
...
func getHost(ctx context.Context, rows rowsInterface) string {
	// these are the fields in the table output from SELECT * FROM information_schema.PROCESSLIST;
...
func (m *MySQLMetrics) CollectMetricsOnce(ctx context.Context) (*workloadmanager.WorkloadMetrics, error) {
	...
	currentRole := m.getCurrentRole(ctx)
