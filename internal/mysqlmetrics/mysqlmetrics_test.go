...
func TestGetCurrentRole(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			got := m.getCurrentRole(context.Background())
			if got != tc.want {
	...
	tests := []struct {
	...
	}{
		{
			...
			replicationZonesRows: &replicationZonesRows{
				...
				size:  1,
				data: []sql.NullString{
					sql.NullString{String: "testid"},
					sql.NullString{String: "testuser"},
					sql.NullString{String: "1.2.3.4"},
					sql.NullString{String: "testdb"},
					sql.NullString{String: "testcommand"},
					sql.NullString{String: "testtime"},
					sql.NullString{String: "teststate"},
					sql.NullString{String: "testinfo"},
				},
				shouldErr: false,
			},
			lookupAddrValue: map[string][]string{
